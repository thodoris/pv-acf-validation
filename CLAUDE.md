...
## Hard rules for the pre-boot compatibility gate

The gate lives in `src/main.tsx`, runs BEFORE the App module is imported, and refuses to mount the questionnaire when the browser is missing required capabilities (viewport ≥ 1100px, localStorage, IndexedDB, customElements). Source of truth: `src/lib/compatibility.ts`.

- **The gate is in `main.tsx`, NOT `App.tsx`.** Unlike the admin / submitted / TTL guards (which sit inside `App.tsx`), the compatibility check must precede the App import. Importing `App` transitively loads `src/state/sessionStore.ts` (whose Zustand `persist` middleware reads localStorage at module load) and `src/lib/firebase.ts` (which initialises App Check + Firestore as module side-effects). Putting the gate inside `App` means storage has already been probed and Firebase has booted by the time the gate fires — the gate would be cosmetic only.
- **The `App` import MUST stay dynamic.** `main.tsx` calls `import('./App').then(...)`, not `import App from './App'`. A static import defeats the entire point of the gate: the bundle would eagerly resolve sessionStore + Firebase even on the failure path. The production bundle split confirms this — `App-*.js` (~1 MB with Firebase) is a separate lazy chunk; only the small `index-*.js` (compat + blocker + CSS + React) is eager.
- **Probes are pure and synchronous.** No network, no async, no side-effects beyond the localStorage write+read+remove probe key (`__pvacf_probe`). Adding an async probe would force the gate to either block the page on a network round-trip or race the resize-recovery flow.
- **Auto-recovery is viewport-only, driven by TWO triggers.** A resize listener (`setTimeout(0)`-coalesced) catches window drags; a 1-second `setInterval` poll catches silent changes that don't fire resize (the most common culprit is DevTools device-emulation toggle, which can change `screen.width` without firing a resize event). Both stop the moment `checkCompatibility()` passes. Auto-recovery is only armed when `isViewportOnlyFailure(initial)` was true at boot — storage / IndexedDB / customElements failures require a manual reload.
- **`MIN_VIEWPORT_PX = 1100` is the gate threshold, applied to BOTH `innerWidth` AND `screen.width`.** The viewport probe takes `Math.min(innerWidth, screen.width)`. `innerWidth` alone is fooled by mobile devices: the `<meta name="viewport" content="width=1280">` in `index.html` forces the layout viewport to 1280px regardless of the device's actual screen, so an iPhone reports `innerWidth=1280` while `screen.width=390`. The min catches mobile devices AND narrow browser windows on big monitors. The existing `@media (max-width: 880px)` CSS reflow rules in `styles-phase-a.css` are the prototype's narrow-viewport degradation floor, not a sanctioned mobile layout — the gate is stricter than the CSS deliberately.
- **`<noscript>` in `index.html` is the JS-disabled fallback.** It hides `#root` via inline `<style>` so the empty mount div doesn't appear next to the message. Independent of the React gate; do not delete.
