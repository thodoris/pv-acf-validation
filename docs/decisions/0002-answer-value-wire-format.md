...
- **`lockedAt` semantics — clarified by [ADR 0009](./0009-f4-boundary-shift.md).** As of the F4 boundary shift, `lockAnswer` is overwrite-safe; `lockedAt` records the **latest** save, not the first. The wire format is unchanged, but the field's meaning in the sealed document is "when was this answer last touched" rather than "when was it first locked". Downstream analysis tools should read it accordingly. If first-answer provenance is ever needed, add a separate `firstLockedAt` field then.
